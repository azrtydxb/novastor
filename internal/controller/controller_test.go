...
func TestSharedFilesystemReconciler_CreatesDeploymentAndService(t *testing.T) {
	...
	if deploy.Spec.Template.Spec.Containers[0].Image != nfsFilerImage {
		t.Errorf("expected image %q, got %q", nfsFilerImage, deploy.Spec.Template.Spec.Containers[0].Image)
	}
